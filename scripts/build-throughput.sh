...
for contract in $CONTRACTS_DIR/*.combined.json; do
    ...
    echo "[+] Building contract $contract_id in interpreter mode"

    pushd "$EVM_DIR" >/dev/null

    if print_and_run /usr/bin/time -v -o "$OUT_PREFIX.build.time" \
        ./quick-build.sh afuzz-interp "$contract_id" \
        | tee -a "$OUT_PREFIX.interp.build.log";
    then
    ...
    echo "[+] creating build tarball"
    timed_print_and_run tar --exclude=".git" --exclude=".ccache" -hcf "$OUT_PREFIX.interp.build.tar" .
    timed_print_and_run xz -3 -T 0 "$OUT_PREFIX.interp.build.tar"
