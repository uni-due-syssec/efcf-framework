...
contract Grid {
    ...
    event PixelPrice(uint16 row, uint16 col, address owner, uint256 price);
  ...
  */
    ...
    function Grid() {
        admin = msg.sender;
        // hardcode values from real contract at https://etherscan.io/address/0x9d6cde970c30e45311cff3d1f3f9a8dd24ff70f1#readContract
        defaultPrice = 2000000000000000;
        feeRatio = 40;
        incrementRate = 25;
    }
    ...
    function setFeeRatio(uint256 _feeRatio) onlyAdmin {
        feeRatio = _feeRatio;
    }

    function setDefaultPrice(uint256 _defaultPrice) onlyAdmin {
        defaultPrice = _defaultPrice;
    }
    ...
    function getPixelColor(uint16 row, uint16 col)
        constant
        validPixel(row, col)
        returns (uint24)
    {
    ...
    function getPixelOwner(uint16 row, uint16 col)
        constant
        validPixel(row, col)
        returns (address)
    {
    ...
    function getPixelPrice(uint16 row, uint16 col)
        constant
        validPixel(row, col)
        returns (uint256)
    {
    ...
    function checkPendingWithdrawal() constant returns (uint256) {
    ...
    function buyPixel(
        ...
        uint256 price = getPixelPrice(row, col);
        ...
        uint256 fee = SafeMath.div(msg.value, feeRatio);
        uint256 payout = SafeMath.sub(msg.value, fee);

        uint256 adminBalance = pendingWithdrawals[admin];
        pendingWithdrawals[admin] = SafeMath.add(adminBalance, fee);

        uint256 ownerBalance = pendingWithdrawals[owner];
        ...
        uint256 increase = SafeMath.div(
            SafeMath.mul(price, incrementRate),
            100
        );
