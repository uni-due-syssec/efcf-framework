...
contract WeBetCrypto {
    ...
    function WeBetCrypto() {
        ...
        Transfer(
            selfAddress,
            0x166Cb48973C2447dafFA8EFd3526da18076088de,
            22500000000000
        );
        ...
        Transfer(
            selfAddress,
            0xE59CbD028f71447B804F31Cf0fC41F0e5E13f4bF,
            15000000000000
        );
        ...
        Transfer(
            selfAddress,
            0x1ab13D2C1AC4303737981Ce8B8bD5116C84c744d,
            5000000000000
        );
        ...
        Transfer(
            selfAddress,
            0x06908Df389Cf2589375b6908D0b1c8FcC34721B5,
            2500000000000
        );
        ...
        Transfer(
            selfAddress,
            0xEdBd4c6757DC425321584a91bDB355Ce65c42b13,
            2500000000000
        );
        ...
        Transfer(
            selfAddress,
            0x4309Fb4B31aA667673d69db1072E6dcD50Fd8858,
            2500000000000
        );
    ...
    function transfer(
        ...
        uint256 _value,
        bytes _data
    ) external isRunning requireThaw returns (bool success) {
        if (_to == selfAddress) {
     ...
     */
    function transferToSelf(uint256 _value, bytes _data)
        internal
        returns (bool success)
    {
        balances[msg.sender] = safeSub(balances[msg.sender], _value);
        ...
        allowed[selfAddress][msg.sender] =
            _value +
            allowed[selfAddress][msg.sender];
     ...
     */
    function checkCooldown(address _allower, address _allowee)
        external
        constant
        returns (uint256 remaining)
    {
     ...
     */
    function checkMonthlyLimit(address _owner)
        external
        constant
        returns (uint256 remaining)
    {
     ...
     */
    function claimTokens(address _token) external isAdmin {
        require(_token != selfAddress);
        WeBetCrypto token = WeBetCrypto(_token);
        uint256 balance = token.balanceOf(selfAddress);
     ...
     */
    function emergencyFreeze() external isAdmin {
        isFrozen = true;
    }
     ...
     * @dev To be used only in extreme circumstances
     */
    function emergencyThaw() external isAdmin {
        isFrozen = false;
    }
	 ...
	 */
    function emergencySplitToggle() external isAdmin {
	 ...
	 */
    function setPriceOfEther(uint256 newPrice, string TLSNotaryProof)
        external
        isAdmin
    {
     ...
     */
    function calculateTokenAmount(uint256 _value)
        internal
        returns (uint256 tokenAmount)
    {
        tokenAmount =
            (((_value * (10**7)) / 1 ether) * pricePerEther) /
            getPricePerToken();
    ...
    function splitProfits() external {
        ...
        if (!isFrozen) {
            ...
            currentProfits =
                ((balances[selfAddress] - 30000000000000) / 10) *
                7;
            amountInCirculation = safeSub(
                300000000000000,
                balances[selfAddress]
            );
            currentIteration = 0;
            actualProfitSplit = 0;
        } else {
            for (i = currentIteration; i < users.length; i++) {
                ...
                if (allowed[selfAddress][users[i]] == 0) {
                    ...
                    (balances[users[i]] / allowed[selfAddress][users[i]]) < 19
                ) {
                    checkSplitEnd(i);
                    continue;
                }
                actualProfitSplit +=
                    (balances[users[i]] * currentProfits) /
                    amountInCirculation;
                balances[users[i]] +=
                    (balances[users[i]] * currentProfits) /
                    amountInCirculation;
                ...
                Transfer(
                    selfAddress,
                    users[i],
                    (balances[users[i]] / amountInCirculation) * currentProfits
                );
            }
        }
    }
    ...
    function ICOSplit() external isAdmin oneTime {
        ...
        if (!isFrozen) {
            ...
            currentProfits =
                ((balances[selfAddress] - 50000000000000) / 10) *
                7;
            amountInCirculation = safeSub(
                300000000000000,
                balances[selfAddress]
    ...
    function isDAppReady() external isAdmin {
        ...
        for (uint256 i = 0; i < devApprovals.length; i++) {
            if (devApprovals[i]) {
                numOfApprovals++;
            }
        }
	 ...
	 */
    function alterBankBalance(
        address _toAlter,
        uint256 _amount,
        bool sign
    ) external DAppOnline isAdmin {
        if (
            sign &&
            (_amount + allowed[selfAddress][_toAlter]) >
            allowed[selfAddress][_toAlter]
        ) {
            allowed[selfAddress][_toAlter] =
                _amount +
                allowed[selfAddress][_toAlter];
            ...
            allowed[selfAddress][_toAlter] = safeSub(
                allowed[selfAddress][_toAlter],
                _amount
            );
            Approval(selfAddress, _toAlter, allowed[selfAddress][_toAlter]);
