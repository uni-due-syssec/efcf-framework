...
contract CommonWallet {
    ...
    function depoToken(address tokenAddr, uint256 amount) public {
        if (ERC20Token(tokenAddr).transferFrom(msg.sender, this, amount)) {
            tokenBalance[tokenAddr][msg.sender] = safeAdd(
                tokenBalance[tokenAddr][msg.sender],
                amount
            );
        }
    }

    function wdEther(uint256 amount) public {
    ...
    function wdToken(address tokenAddr, uint256 amount) public {
        ...
        if (ERC20Token(tokenAddr).transfer(msg.sender, amount)) {
            tokenBalance[tokenAddr][msg.sender] = safeSub(
                tokenBalance[tokenAddr][msg.sender],
                amount
            );
        }
    ...
    function getTokenBalance(address tokenAddr, address user)
        public
        view
        returns (uint256)
    {
    ...
    function sendEtherTo(address to_, uint256 amount) public {
        require(etherBalance[msg.sender] >= amount);
