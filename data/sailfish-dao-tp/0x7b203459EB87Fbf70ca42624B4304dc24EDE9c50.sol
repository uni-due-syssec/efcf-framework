contract Partner {
    function exchangeTokensFromOtherContract(
        address _source,
        address _recipient,
...
contract COE {
    ...
    uint256 tierLevel = 0;
    uint256 fiatPerEth = 385000000000000000000000; // cents per ETH in this case (*10^18)
    ...
    function allocateTokens(uint256 _submitted) internal {
        uint256 _availableInTier = mul(
            tierTokens[tierLevel],
            costPerToken[tierLevel]
        );
        ...
        if (_submitted >= _availableInTier) {
            ...
            uint256 _tokens = div(
                div(mul(_submitted, 1 ether), costPerToken[tierLevel]),
                1 ether
            );
    ...
    function requestTokensFromOtherContract(
    ...
    function canContractExchange(address _contract)
        public
        constant
        returns (bool)
    {
    ...
    function handleTokensFromOtherContracts(
        address _contract,
        address _recipient,
        uint256 _tokens
    ) {
